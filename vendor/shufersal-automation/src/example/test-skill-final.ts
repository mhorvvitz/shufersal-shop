...
// Simulate: "Add milk, apples, shredded cheese, sliced cheese, greek yogurt and pita"
const requests = [
  { query: 'milk', qty: null },
  { query: 'apples', qty: null },
  { query: 'shredded cheese', qty: null },
  { query: 'sliced cheese', qty: null },
  { query: 'greek yogurt', qty: null },
  { query: 'pita', qty: null },
];
...
async function main() {
  ...
  for (const req of requests) {
    ...
    if (entry) {
      ...
      matched.push({ entry, qty });
    } else {
      console.log(`  ${req.query} -> NOT FOUND in dictionary`);
      unmatched.push(req.query);
    }
  }

  if (matched.length === 0) {
    console.log('\nNo products matched. Nothing to add.');
    return;
  }
  ...
  try {
    ...
    if (unmatched.length > 0) {
      console.log(`\nNot in dictionary (add manually): ${unmatched.join(', ')}`);
    }
